...
component {
    ...
    /**
     * Set binary data for binary results
     * @binaryData The PDF binary data
     */
    public PDFResult function setBinaryData( required any binaryData ) {
        variables.binaryData = arguments.binaryData;
        return this;
    }
    ...
    public string function toString() {
        if ( !variables.success ) {
            return "PDFResult[success=false, error=#variables.error#]";
        }
        
        var hasFile = len( variables.filePath ) > 0;
        var hasBinary = !isSimpleValue( variables.binaryData ) || len( variables.binaryData ) > 0;
        
        return "PDFResult[success=true, " &
               "output=" & ( hasFile ? "file" : ( hasBinary ? "binary" : "none" ) ) & ", " &
               "size=#getFormattedFileSize()#, " &
               "time=#getFormattedGenerationTime()#, " &
               "pages=#variables.pageCount#]";
    }

    // Getters
    public boolean function getSuccess() { return variables.success; }
    public string function getFilePath() { return variables.filePath; }
    public any function getBinaryData() { return variables.binaryData; }
    public numeric function getFileSize() { return variables.fileSize; }
    public numeric function getGenerationTime() { return variables.generationTime; }
    public numeric function getPageCount() { return variables.pageCount; }
    public string function getError() { return variables.error; }
    public string function getErrorDetail() { return variables.errorDetail; }
