...
component extends="coldbox.system.EventHandler" {
    ...
    function binary(event, rc, prc) {
        try {
            ...
            // Generate PDF
            var result = pdfGeneratorService.htmlToPDFResult(requestData.html, pdfOptions);
            ...
            event.renderData(type = 'binary', data = result.getBase64String());
        } catch (any e) {
        ...
        try {
            ...
            event.renderData(
                ...
                data = {
                    ...
                    'version': 'v0.1.0',
                    'timestamp': now(),
                    'checks': {'pdfGeneration': isHealthy}
                },
    ...
    function test(event, rc, prc) {
        ...
        try {
            ...
            // Generate PDF
            var result = pdfGeneratorService.htmlToPDFResult(testHTML, pdfOptions);
            ...
            event.renderData(type = 'binary', data = toBinary(result.getBase64String()));
        } catch (any e) {
    ...
    private PDFOptions function buildPDFOptions(required struct optionsData) {
        ...
        // Set margins
        if (
            structKeyExists(arguments.optionsData, 'marginTop') ||
